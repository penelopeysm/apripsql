...
instance (Csv.DefaultOrdered a) => Csv.DefaultOrdered (WithId a) where
  headerOrder a = V.cons "id" (Csv.headerOrder a)

-- | Write a list of values to a CSV file, with each value having an
-- autogenerated ID (starting from 1)
toIdCsv :: (Csv.ToNamedRecord a, Csv.DefaultOrdered a) => FilePath -> [a] -> IO ()
toIdCsv fname values =
  toCsv fname $ zipWith WithId [1 ..] values
